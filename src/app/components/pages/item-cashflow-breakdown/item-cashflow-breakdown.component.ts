...
@Component({
  selector: 'app-item-cashflow-breakdown',
  templateUrl: './item-cashflow-breakdown.component.html',
  styleUrls: ['./item-cashflow-breakdown.component.scss']
})
export class ItemCashflowBreakdownComponent implements OnInit {
  ...
  private createChartData() {
    this.yearlyItemDataMap.forEach((itemDataMap: Map<string, ItemData>, year: string) => {
      const itemCashflowListForYear = [];
      itemDataMap.forEach((itemData: ItemData, itemName: string) => {
        // We will include the cash inflow in the legend for readability.
        const formattedName = itemName + ' ($' + itemData.cashInflow + ')';
        const chartDataItem = {
          name: formattedName,
          value: itemData.cashInflow
        }
        itemCashflowListForYear.push(chartDataItem);
      });
      this.yearData[year] = itemCashflowListForYear;
    });
  }
