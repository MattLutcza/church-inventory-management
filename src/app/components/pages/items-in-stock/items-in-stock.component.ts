...
export class ItemsInStockComponent implements OnInit {
  ...
  ngOnInit() {
    this.route.data.subscribe(data => {
      this.inventoryItemQuantities = this.determineUnsoldItemQuantities(data.inventoryItems);
    });
  }

  private determineUnsoldItemQuantities(inventoryItems: InventoryItem[]): Map<string, number> {
